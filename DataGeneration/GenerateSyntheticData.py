...
def generate_data(data, G, exog, lag, std_metrics, regressors):
    ...
    for t in tqdm(range(lag, len(exog))):
...
def main():
    ...
    exog_df = df[['timestamp', 'workload']].astype(float).groupby('timestamp').agg(sum).reset_index(drop=True)
    ...
    for i in range(RUNS):
        print(i, end='\r')
        path = os.path.join(write_dir, 'Graph{}.gpickle'.format(i))

        G = read_call_graph(path)

        exog_services = [n for n,d in G.in_degree() if d==0]
        exog = pd.DataFrame({i : abs(np.random.normal(exog_df['workload'].mean(), exog_df['workload'].std(), size=len(exog_df))) for i in exog_services})

        data = initialize_data(G)

        data = generate_data(data, G, exog, lag, std_metrics, regressors)
        
        DIR = os.path.join(wrie_dir, 'synthetic', 'Graph{}'.format(i))
